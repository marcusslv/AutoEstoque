...
import type { StockMovementFormValues, StockOutputType } from '../types/movement'
...
<template>
  <EntityFormDialog
    ...
    description="Registre baixa manual por consumo, perda, quebra ou ajuste."
    submit-label="Registrar saida"
    ...
    <div class="space-y-4">
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        <FormField label="Tipo" required :error="fieldError('type')">
          <AppSelect
            ...
            :options="typeOptions"
            @update:model-value="form.type = $event as StockOutputType"
          />
        </FormField>
      ...
      <FormField label="Motivo" required :error="fieldError('reason')">
        <AppInput
          v-model="form.reason"
          autocomplete="off"
          placeholder="Consumo em servico"
        />
      </FormField>
      ...
      <FormField label="Observacao" :error="fieldError('note')">
        <AppTextarea
          v-model="form.note"
          :rows="3"
          placeholder="OS, responsavel ou contexto da baixa"
        />
      </FormField>
    </div>
  </EntityFormDialog>
